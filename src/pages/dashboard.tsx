...
import { useRoqPlatformUserProfiles, useOperationsDashboardCount, useLocationCount, useCarCount } from 'lib/roq';

import { OperationsDashboardListPage as ShowCaseList } from 'pages/operations-dashboards';

function DashboardPage() {
  const { session } = useSession();

  const operationsDashboardStat = useOperationsDashboardCount();

  const locationStat = useLocationCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: operationsDashboardStat?.data,
      isLoading: operationsDashboardStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Locations',
      result: locationStat?.data,
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Operations Dashboards
                </Text>
                <NextLink href={`operations-dashboards`} passHref legacyBehavior>
                  <Button
