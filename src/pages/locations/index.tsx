...
import { useRoqClient, useLocationFindManyWithCount } from 'lib/roq';
...
export function LocationListPage(props: LocationListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['address.contains', 'city.contains', 'state.contains', 'country.contains'],
          ...params.filters,
        },
        'location',
      ),
  ...
  const { data, error, isLoading, mutate } = useLocationFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.location.delete({ where: { id } });
      await mutate();
    } catch (error) {
